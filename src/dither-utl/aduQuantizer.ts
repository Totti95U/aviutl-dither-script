...
class AduQuantizer {
    ...
    addColor(color: Oklab) {
        this.colorSet.push(color);
    }

    async addPixels(colorData: Uint8Array) {
    ...
    async reducePalette(targetColors: number, progressCallback: (progress: number) => void) {
        const self = this;
        const maxEpochs = 1e8;
        const eps = 0.0001;

        function sendProgress(epoch: number) {
     ...
     */
    findClosestClusterIdx(color: Oklab) {
    ...
    async generatePalette(colorCount: number, progressCallback: (progress: number) => void) {
        await this.reducePalette(colorCount, progressCallback);
        return this.getPalette();
    }

    async applyPalette(pixelData: Uint8Array) {
